...
void ComputeForces()
{
  ...
  while(loop--){
    ...
    MPI_Send(&pv, sizeof(struct ParticleV), MPI_CHAR, 0, MPI_ANY_TAG, MPI_COMM_WORLD);
    MPI_Send(&max_f, 1, )
  }
    pthread_exit(NULL);
}
...
void ComputeNewPos()
{
  ...
  while(loop--){
    ...
    MPI_Recv(&pv_servo, sizeof(struct ParticleV), MPI_CHAR, rank, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    //sem_wait(&newPos);
    //printf("NEW\n");
    ...
    for (int rank = 1; rank < numt; rank++) {
        MPI_Send(&particles, sizeof(struct Particle), MPI_CHAR, rank, MPI_ANY_TAG, MPI_COMM_WORLD);
        MPI_Send(&pv, sizeof(struct ParticleV), MPI_CHAR, rank, MPI_ANY_TAG, MPI_COMM_WORLD);
    }
  }
}
...
int main(int argc, char **argv)
{
    ...
    if(argc != 3){
  	...
  	if (rank == 0) {
        ...
        pv_servo = (ParticleV *) malloc(sizeof(ParticleV)*tamanho_laco);
	}
